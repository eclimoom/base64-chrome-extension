...
const DEMO_PATH =
  'http://192.168.6.35/predicts/1.2.156.600734.762202257.139620.1730459902.8499.5400253/biomind/98117e9d-a22b-11ef-a3bc-1b7ba874f173/mask/404754_20.png';
...
export class BaseToPngComponent implements AfterViewInit {
  urlPath: any =
    'http://192.168.6.34/bm/29b0ed5e-6bead6e2-17e95725-a216d7a8-b60706a8/14b07388-7c10b956-82568724-c450fba3-56eb2069/8e85119c-b7c8b53a-09930fc9-59dd9faa-6f2aa338_0?__8e85119c-b7c8b53a-09930fc9-59dd9faa-6f2aa338/0/high-quality';
  loading = false;
  ...
  getHttpData(url = '', token = ''): void {
    this.fetchService.getFetchData(url, token).subscribe(async (res: any) => {
      ...
      const { type, isImg } = this.fetchService.getFileType(
        view.slice(0, 35),
        view.slice(-24)
      );
      console.log('type:', type, isImg);
      if (isImg) {
        const image = this.fetchService.decodeImage(view, type);
        if (image) {
  ...
  _renderPixel({ pixelData, width, height }): string {
    const canvas: any = document.createElementNS(
