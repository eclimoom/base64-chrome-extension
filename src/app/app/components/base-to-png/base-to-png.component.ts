import {AfterViewInit, Component} from '@angular/core';
import {DataFetchService} from '../../services/data-fetch.service';
import {PNG} from 'biomind-png-js';
import * as cornerstone from 'cornerstone-core';
import * as cornerstoneTools from 'cornerstone-tools';
import * as cornerstoneMath from 'cornerstone-math';
import Hammer from 'hammerjs';

const STORAGE_BASE_NAME = 'storageBase64';
const DEMO_STR = '';
// const DEMO_PATH = 'https://srv.carbonads.net/static/30242/15f7619952d3cdf0e1f51f9262d4afe75069da8a';
// const DEMO_PATH = 'http://devfront.top/public/images/2938.jpg';

// mask
// tslint:disable-next-line:max-line-length
const DEMO_PATH = 'http://192.168.6.35/predicts/1.2.156.600734.762202257.139620.1730459902.8499.5400253/biomind/98117e9d-a22b-11ef-a3bc-1b7ba874f173/mask/404754_20.png';

// bm
// tslint:disable-next-line:max-line-length
// const DEMO_PATH = 'http://192.168.6.35/bm/aacc748f-07a65df0-83053431-72c4d111-21f4cf0a/010bf150-f4eab77a-6a9f7301-33739b7e-82030c25/4bf4f05b-cb836d89-d127a2af-031c6553-a8963418_0';


@Component({
  selector: 'app-base-to-png',
  templateUrl: './base-to-png.component.html',
  styleUrls: ['./base-to-png.component.scss']
})
export class BaseToPngComponent implements AfterViewInit {

  urlPath: any = 'http://192.168.6.34/predicts/1648606947.2521474.20079.11/biomind/609dacbb-bc71-11ef-8d28-fd21f6371a58/mask/67761_15.png';
  loading = false;

  pngConfig = {
    baseString: '',
    imgType: '',  // 图像类型
    backgrounds: [
      {color: '#000', label: '黑'},
      {color: '#fff', label: '白'},
      {color: '#f00', label: '红'},
    ],
    background: '',
  };
  imgTypes = [
    {type: 'string', name: '字符串'},
    {type: 'url', name: 'url'}
  ];

  baseString: string;

  list: any[];

  constructor(private fetchService: DataFetchService) {
    this.pngConfig.imgType = this.imgTypes[1].type;
    this.pngConfig.background = this.pngConfig.backgrounds[0].color;
    this.list = this.getBase64List();
  }

  ngAfterViewInit(): void {
    setTimeout(() => {
      this.setSelectionRange();
    }, 0);
    cornerstoneTools.external.cornerstone = cornerstone;
    cornerstoneTools.external.cornerstoneMath = cornerstoneMath;
    cornerstoneTools.external.Hammer = Hammer;
    cornerstoneTools.init({
      showSVGCursors: true,
    });
  }

  loadData(): void {
    this.getHttpData(this.urlPath);
    // this.fetchService.getFetchData(DEMO_PATH);
  }

  getHttpData(url = '', token = ''): void {
    
    this.fetchService.getFetchData(url, token).subscribe(async (res: any) => {
      const view = new Uint8Array(res);

      const {type, isImg} = this.fetchService.getFileType(view.slice(0, 35), view.slice(-24));
      console.log('type:', type, isImg);
      let image: any;
      if (isImg) {
        // 1. 解码图片
        if (['dicom_png', '1png', 'png'].includes(type)) {
          // @ts-ignore
          const png = new PNG(view);
          const pngPixelData = png.decodePixels();
          let pixelData: any;
          if (type === 'dicom_png') {
            const pixelDataOriginal = new Int16Array(png.width * png.height);
            for (let i = 0; i < png.width * png.height; i++) {
              pixelDataOriginal[i] = pngPixelData[2 * i] * 256 + pngPixelData[2 * i + 1];
            }
            pixelData = new Int16Array(pixelDataOriginal.buffer);
            const wcDefault = 350;
            const wwDefault = 1000;
            const width = png.width;
            const height = png.height;
            const imageId = png.imageId;

            image = {
              imageId,
              _dicomImage: true,
              minPixelValue: -32768,
              maxPixelValue: 32767,
              slope: 1.0,
              intercept: 0,
              windowCenter: wcDefault,
              windowWidth: wwDefault,
              render: cornerstone.renderGrayscaleImage,
              getPixelData: () => pixelData,
              rows: height,
              columns: width,
              height,
              originalWidth: width,
              originalHeight: height,
              width,
              color: false,
              columnPixelSpacing: 1.0,
              rowPixelSpacing: 1.0,
              invert: false,
              sizeInBytes: width * height * 2,
            };
          } else {
            const area = png.width * png.height;
            const color = [255, 255, 0];

            let whiteBoard: any;
            console.log('pngPixelData:', png.pixelBitlength, png);
            if (type === '1png') {
              whiteBoard = this.transform8Uint(pngPixelData, png.width, png.height, color);
            } else {
              whiteBoard = pngPixelData;
            }
            pixelData = new Uint16Array(whiteBoard);

            image = {
              imageId: 'pngImage' + (new Date().getTime()),
              minPixelValue: 0,
              maxPixelValue: 255,
              slope: 1.0,
              intercept: 0,
              windowCenter: 128,
              windowWidth: 256,
              render: cornerstone.renderColorImage,
              getPixelData: () => pixelData,
              rows: png.height,
              columns: png.width,
              height: png.height,
              width: png.width,
              color: true,
              columnPixelSpacing: 1.0,
              rowPixelSpacing: 1.0,
              invert: false,
              sizeInBytes: area * 4,
            };
          }
        } else {
          console.log('type not support', type);
        }
        // 2. 渲染图片
        if (image) {
          const element = document.getElementById('dicomImage');
          try {
            cornerstone.getEnabledElement(element);
          } catch (e) {
            cornerstone.enable(element);
            element.oncontextmenu = (e: any) => e.preventDefault();
            if (!cornerstoneTools.getToolForElement(element, 'Wwwc')) {
              cornerstoneTools.addTool(cornerstoneTools.WwwcTool);
              cornerstoneTools.setToolActive('Wwwc', {mouseButtonMask: 1});

              cornerstoneTools.addTool(cornerstoneTools.ZoomTool);
              cornerstoneTools.setToolActive('Zoom', {mouseButtonMask: 2});
            }
          }
          cornerstone.displayImage(element, image);
        }

      }
    });
  }

  pngVal(): void {
    // console.log("origin: ",this.pngConfig.baseString);
    let str = this.pngConfig.baseString;
    if (!str) {
      this.baseString = null;
      return;
    }
    // str 删除前后空格，删除前后引号 前后换行符
    str = str.trim().replace(/^["'\n]+|["'\n]+$/g, '');
    this.baseString = str;
    console.log('baseString:', this.baseString);
    this.decode();
  }


  decode(): string {
    if (!this.baseString) {
      return '';
    }
    let data = 'data:image/jpeg;base64,';
    if (this.baseString.includes('base64,')) {
      if (!this.baseString.includes('data:')) {
        data = 'data:' + this.baseString;
      } else {
        data = this.baseString;
      }
    } else if (this.pngConfig.imgType === 'string') {
      data += this.baseString;
    } else {
      let png: any;
      let array: Uint8Array;
      try {
        array = this._base64ToUint8Array(this.baseString);
      } catch (e) {
        console.log('decode base64 error, ', e);
        return;
      }

      let width: any;
      let height: any;
      let pixelDataOriginal: number[] | Uint8Array;
      // https://www.cnblogs.com/zhangnan35/p/12433201.html
      if (this.pngConfig.imgType === 'Uint8') {

        try {
          png = PNG.newPng(array);
        } catch (e) {
          console.log('decode png error, ', e);
          return;
        }
        width = png.width;
        height = png.height;
        // console.log("array", png, width, height);
        const decodePixels = png.decodePixels();
        let buffData;
        let currentColor;
        const colorDml = [255, 0, 0];
        const colorDisease = [255, 255, 0];
        switch (png.pixelBitlength) {
          case 8:
            // grayscale
            pixelDataOriginal = new Uint8Array(png.width * png.height * 4);
            buffData = new Uint8Array(decodePixels.buffer);

            for (let i = 0; i < png.width * png.height; i++) {
              if (buffData[i] > 0) {
                // lable2动脉瘤(红色),lable1血管(绿色)
                if (buffData[i] === 2) {
                  currentColor = [...colorDml];
                } else {
                  currentColor = [...colorDisease];
                }
                pixelDataOriginal[i * 4] = currentColor[0];
                pixelDataOriginal[i * 4 + 1] = currentColor[1];
                pixelDataOriginal[i * 4 + 2] = currentColor[2];
                pixelDataOriginal[i * 4 + 3] = 255;
              }
            }
            break;
          default:
            break;
        }
      } else {
      }
      data = this._renderPixel({pixelData: pixelDataOriginal, width, height});
    }
    return data;
  }

  // getPixelValues(pixelData): any {
  //   let minPixelValue = Number.MAX_VALUE;
  //   let maxPixelValue = Number.MIN_VALUE;
  //   const len = pixelData.length;
  //   let pixel = void 0;
  //
  //   for (let i = 0; i < len; i++) {
  //     pixel = pixelData[i];
  //     minPixelValue = minPixelValue < pixel ? minPixelValue : pixel;
  //     maxPixelValue = maxPixelValue > pixel ? maxPixelValue : pixel;
  //   }
  //
  //   return {
  //     minPixelValue,
  //     maxPixelValue
  //   };
  // }

  // str2ab(str) {
  //   const buf = new ArrayBuffer(str.length * 2); // 2 bytes for each char
  //   const bufView = new Array(buf);
  //   for (let i = 0, strLen = str.length; i < strLen; i++) {
  //     bufView[i] = str.charCodeAt(i);
  //   }
  //   return bufView;
  // }
  //
  // buffer2Base64(int16) {
  //   // const binary = String.fromCharCode.apply(null, buffer);
  //   // return window.btoa(binary);
  //   return window.btoa(int16.reduce(
  //     function (data, byte) {
  //       return data + String.fromCharCode(byte);
  //     },
  //     ''
  //   ));
  // }

  _base64ToUint8Array(base64String: string): Uint8Array {
    const rawData = window.atob(base64String) || '';
    const outputArray = new Uint8Array(rawData.length);
    for (let i = 0; i < rawData.length; ++i) {
      outputArray[i] = rawData.charCodeAt(i);
    }
    return outputArray;
  }

  _renderPixel({pixelData, width, height}): string {
    const canvas: any = document.createElementNS('http://www.w3.org/1999/xhtml', 'canvas');
    const ctx = canvas.getContext('2d', {alpha: false});
    const imageData = new ImageData(new Uint8ClampedArray(pixelData), width, height);
    ctx.putImageData(imageData, 0, 0);
    return canvas.toDataURL();
  }

  setSelectionRange(): void {
    const input = document.querySelector('textarea');
    input?.select();
    if (document.execCommand) {
      document.execCommand('paste');
    }

    setTimeout(() => {
      this.pngVal();
    }, 10);
  }

  onImageLoad(): void {
    // 将解析成功的图片添加到历史
    this.setBase64List(this.baseString);
  }

  onImageError($event: Event): void {
    console.log('onImageError', $event);
  }

  getBase64List(): Array<any> {
    return JSON.parse(localStorage.getItem(STORAGE_BASE_NAME) || '[]');
  }

  setBase64List(base64: string): void {
    const list = this.getBase64List();
    if (list.find((item) => item.base64 === base64)) {
      return;
    }
    list.unshift({base64, date: new Date()});
    if (list.length > 5) {
      list.length = 5;
    }
    localStorage.setItem(STORAGE_BASE_NAME, JSON.stringify(list));
  }

  handleSample(): void {

    if (this.pngConfig.imgType === 'string') {
      this.pngConfig.baseString = DEMO_STR;
      this.baseString = DEMO_STR;
      this.decode();
    } else {
      this.urlPath = DEMO_PATH;
      this.urlVal();
    }
  }

  // 上传文件
  handleDrop(event: DragEvent): void {
    event.preventDefault();
    event.stopPropagation();
    if (event.dataTransfer && event.dataTransfer.files.length > 0) {
      const file = event.dataTransfer.files[0];
      this.renderFile(file);
    }
  }


  urlVal(): void {
    this.loading = true;
    const url = this.urlPath;
    if (!url) {
      this.loading = false;
      return;
    }
  }


  private renderFile(file: File): void {
    const reader = new FileReader();
    reader.onload = (e: any) => {
      this.pngConfig.baseString = e.target.result;
      this.pngVal();

    };
    if (file.type.includes('image')) {
      reader.readAsDataURL(file);
    } else {
      reader.readAsText(file);
    }
  }

  handleTypeChange(): void {
    if (this.pngConfig.imgType === 'url') {
      this.baseString = '';
      this.urlPath = 'http://192.168.6.35/predicts/1.2.156.600734.762202257.139620.1730459902.8499.5400253/biomind/98117e9d-a22b-11ef-a3bc-1b7ba874f173/mask/404754_13.png';
    } else {
      this.urlPath = '';
    }
  }

  transform8Uint(oneBytePixels: any, width: number, height: number, colorDisease: any): any {
    const pixelDataOriginal = new Uint8Array(width * height * 4);
    const b = [1, 2, 4, 8, 16, 32, 64, 128];

    const redu = width % 8;
    let widthPNG = redu ? width + (8 - redu) : width;
    widthPNG = widthPNG / 8;

    for (let y = 0; y < height; y++) {
      const linePos = y * width * 4;
      for (let x = 0; x < widthPNG; x++) {
        const idxPNG = y * widthPNG + x;
        let idxLine = x * 8;
        let idxRGBA = linePos + idxLine * 4;
        // 高位到低位对应像素
        // for (let iBit = 0; iBit < 8; iBit++) {
        // 这个是从低位到高位对应像素
        for (let iBit = 7; iBit >= 0; iBit--) {
          // tslint:disable-next-line:no-bitwise
          if (oneBytePixels[idxPNG] & b[iBit]) {
            pixelDataOriginal[idxRGBA] = colorDisease[0];
            pixelDataOriginal[idxRGBA + 1] = colorDisease[1];
            pixelDataOriginal[idxRGBA + 2] = colorDisease[2];
            pixelDataOriginal[idxRGBA + 3] = 255;
          }
          idxRGBA += 4;
          idxLine += 4;
          if (idxLine >= width * 4) {
            break;
          }
        }
      }
    }
    return pixelDataOriginal;
    // const whiteBoard: Uint8Array = new Uint8Array(imageWidth * imageHeight);
    // const b: number[] = [1, 2, 4, 8, 16, 32, 64, 128];
    //
    // const redu = imageWidth % 8;
    // let widthPNG = redu ? imageWidth + (8 - redu) : imageWidth;
    // widthPNG = widthPNG / 8;
    //
    // for (let y = 0; y < imageHeight; y++) {
    //   const linePos = y * imageWidth;
    //   for (let x = 0; x < widthPNG; x++) {
    //     const idxPNG = y * widthPNG + x;
    //     let idxLine = x * 8;
    //     let idxRGBA = linePos + idxLine;
    //     // 高位到低位对应像素
    //     // for (let iBit = 0; iBit < 8; iBit++) {
    //     // 这个是从低位到高位对应像素
    //     for (let iBit = 7; iBit >= 0; iBit--) {
    //       // tslint:disable-next-line:no-bitwise
    //       if (oneBytePixels[idxPNG] & b[iBit]) {
    //         whiteBoard[idxRGBA] = 1;
    //       }
    //       idxRGBA++;
    //       idxLine++;
    //       if (idxLine >= imageWidth) {
    //         break;
    //       }
    //     }
    //   }
    // }
    // return whiteBoard;
  }
}
